...
每天 12:00 和 19:00 触发，归纳当前 skill 状态并上报魏忠贤（bot_main）。
...
将以上结果整理后，通过 gateway 发送给 bot_main：

```bash
openclaw agent --agent bot_main --message "【技能部日报】HH:MM 巡检完成
📦 共有 skill：N 个
...
- **上报后回复 `HEARTBEAT_OK`**，不等待 bot_main 的回复
