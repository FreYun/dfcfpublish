...
7. **若本次要写稿、发小红书或回复评论**：先读 `memory/发帖记录.md` — 了解以前都发过什么；再读 `memory/写稿经验.md` — 吸取历次草稿修改的教训；再读 `memory/小红书拉近距离的表达方式.md` — 参考拉近距离的开场和措辞技巧。三个文件读完后再动笔，保证连续性、不重蹈覆辙
...
你的小红书 MCP 服务端口是 **18065**（容器 `xiaohongshu-mcp-5`，端点 `http://localhost:18065/mcp`）。**浏览、搜索、回复评论等非发布操作**参考 `skills/xiaohongshu-mcp/SKILL.md`。
...
> 素材库文件：`skills/xhs-content-planner/topic-library.md`
